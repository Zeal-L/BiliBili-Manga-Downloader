...
from src.utils import (MAX_RETRY_LARGE, MAX_RETRY_SMALL, RETRY_WAIT_EX,
                       TIMEOUT_LARGE, TIMEOUT_SMALL, __app_name__,
                       __copyright__, __version__, logger)
...
class Episode:
    """漫画章节类，用于管理漫画章节的详细信息
    """
    def __init__(self, episode: dict, sessData: str, comic_id: str, comic_info: dict, mainGUI: MainGUI) -> None:
    ...
    def saveToPDF(self, mainGUI: MainGUI, temp_imgs: list[Image.Image]) -> None:
        ...
        def _():
            try:
                temp_imgs[0].save(self.epi_path_pdf, save_all=True, append_images=temp_imgs[1:], quality=95)
    ...
    def saveToFolder(self, mainGUI: MainGUI, temp_imgs: list[Image.Image]) -> None:
        ...
        def _():
            try:
                for index, img in enumerate(temp_imgs, start=1):
                    ...
                    info = {}
                    info[piexif.ImageIFD.ImageDescription] = f'《{self.comic_name}》 - {self.title}'.encode('utf-8')
                    info[piexif.ImageIFD.Artist] = self.author.encode('utf-8')
                    info[piexif.ImageIFD.Software] = f"{__app_name__} {__version__}".encode('utf-8')
                    info[piexif.ImageIFD.Copyright] = __copyright__.encode('utf-8')
                    exif_bytes = piexif.dump({'0th': info})
                    img.save(os.path.join(self.epi_path_folder, f'{str(index).zfill(3)}.jpg'), exif=exif_bytes)
        ...
        try:
            ...
            logger.exception(e)
            mainGUI.message_box.emit(f"《{self.comic_name}》章节：{self.title} 保存图片到文件夹多次后失败!\n已暂时跳过此章节!\n请重新尝试或者重启软件!\n\n更多详细信息请查看日志文件, 或联系开发者！")
    ...
    def saveTo7z(self, mainGUI: MainGUI, temp_imgs: list[Image.Image]) -> None:
            ...
            try:
                with SevenZipFile(f'{self.epi_path_7z}', 'w') as z:
                    ...
                    for root, _dirs, files in os.walk(self.epi_path_folder):
                        for file in files:
                            z.write(os.path.join(root, file), os.path.basename(os.path.join(root, file)))
        ...
        try:
            ...
            logger.exception(e)
            mainGUI.message_box.emit(f"《{self.comic_name}》章节：{self.title} 保存图片到7z多次后失败!\n已暂时跳过此章节!\n请重新尝试或者重启软件!\n\n更多详细信息请查看日志文件, 或联系开发者！")
    ...
    def isDownloaded(self) -> bool:
        ...
        """
        return os.path.exists(self.epi_path_pdf) \
